...
for k in range(N * N):
    ...
    # 次に搬出すべき箱 k_next を探す
    k_next = k + 1
    while k_next < N * N and k_next not in pos:
        k_next += 1
    k_next = k_next if k_next < N * N else None

    # ステップB: 外周コンベアで出口へ移動（k_next の縦移動をインターリーブ）
    i2, j2 = pos[k]
    ...
    if fwd == 0:
        ...
        peri_d = 1 if fwd <= bwd else -1
        peri_steps = fwd if fwd <= bwd else bwd
        for step in range(peri_steps):
            apply_op(0, peri_d)
            if k not in pos:
                break
            # k_next の縦移動をインターリーブ（フリーライドが逆方向になるperi_d=-1は除く）
            if peri_d == 1 and k_next is not None and k_next in pos:
                i1, j1 = pos[k_next]
                if i1 > 0 and 1 <= j1 <= N - 2:
                    m1, jl1, jr1 = find_vertical_conveyor(j1)
                    d1 = -1 if j1 == jl1 else 1
                    ki, kj = pos[k]
                    # k が k_next の縦コンベアと共有するマスにいる場合はスキップ
                    shared = {(0, jl1), (0, jr1), (N - 1, jl1), (N - 1, jr1)}
                    # row0 の箱が内部へ押し出される場合はスキップ
                    danger_cell = (0, jr1) if d1 == -1 else (0, jl1)
                    # 残り外周ステップ内にrow0到達できる場合のみ（フリーライド保証）
                    remaining = peri_steps - step - 1
                    if ((ki, kj) not in shared and
                            board[danger_cell[0]][danger_cell[1]] is None and
                            i1 <= remaining):
                        apply_op(m1, d1)
